...
public class XmlKeyboardLoader {
    ...
    public SoftKeyboard loadKeyboard(int resourceId) {
        MMLog.log(TAG, "loadKeyboard loading ... ...");
        ...
        try {
                ...
                if (mXmlEventType == XmlResourceParser.START_TAG) {
                    ...
                    if (XMLTAG_KEYBOARD.compareTo(attr) == 0) { // keyboard
                        ...
                        if (softKeyboard == null) {
                            MMLog.log(TAG, "XMLTAG_ROW softKeyboard null");
                            return null;
                        }
                        ...
                        if (null == splitter || null == labels) {
                            MMLog.log(TAG, "XMLTAG_KEYS splitter or labels null");
                            return null;
                        }
                        ...
                        if (null == softKeyboard) {
                            MMLog.log(TAG, "XMLTAG_KEY softKeyboard null");
                            return null;
                        }
            ...
            MMLog.log(TAG, "loadKeyboard load over");
            return softKeyboard;
        } catch (XmlPullParserException e) {
            e.printStackTrace();
            MMLog.e(TAG, "loadKeyboard XmlPullParserException " + e.getMessage());
        } catch (IOException e) {
            e.printStackTrace();
            MMLog.e(TAG, "loadKeyboard IOException " + e.getMessage());
        } finally {
            if (xrp != null)
                xrp.close();
        }
